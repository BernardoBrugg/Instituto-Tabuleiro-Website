...
export default function Header() {
  ...
  return (
    <header className="flex items-center justify-between w-full mb-4 relative z-50">
      ...
      {/* Desktop: Navegação + Instagram juntos */}
      ...
      <div className="md:hidden relative z-50">
        ...
        {isOpen && (
          <div className="fixed top-16 right-4 bg-white/95 backdrop-blur-md border border-gray-300 rounded-lg p-4 flex flex-col gap-4 z-[100] shadow-xl min-w-[180px]">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-sm text-black hover:text-gray-600 hover:scale-105 transition-all duration-500 cursor-pointer"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 text-sm text-black hover:text-gray-600 hover:scale-105 transition-all duration-500 cursor-pointer"
              onClick={() => setIsOpen(false)}
            >
